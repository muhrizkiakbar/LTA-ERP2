array:302 [ // app/Http/Controllers/Sap/ReturnController.php:427
  "odata.metadata" => "https://192.168.1.81:50000/b1s/v1/$metadata#CreditNotes/@Element"
  "odata.etag" => "W/"356A192B7913B04C54574D18C28D46E6395428AB""
  "DocEntry" => 447600
  "DocNum" => 2110096913
  "DocType" => "dDocument_Items"
  "HandWritten" => "tNO"
  "Printed" => "psNo"
  "DocDate" => "2024-03-04T00:00:00Z"
  "DocDueDate" => "2024-03-25T00:00:00Z"
  "CardCode" => "C.E.00000124"
  "CardName" => "ERA 5000"
  "Address" => "JL. S. PARMAN MAL LEMBUSWANA BLOK A NO.11 - 12SAMARINDAINDONESIA"
  "NumAtCard" => "TEST_RETURN_NOPUSH_001"
  "DocTotal" => 1130979.89
  "AttachmentEntry" => null
  "DocCurrency" => "IDR"
  "DocRate" => 1.0
  "Reference1" => "2110096913"
  "Reference2" => null
  "Comments" => "Return from ERP - C.E.00000124"
  "JournalMemo" => "A/R Credit Memos - C.E.00000124"
  "PaymentGroupCode" => 4
  "DocTime" => "16:43:00"
  "SalesPersonCode" => 4886
  "TransportationCode" => -1
  "Confirmed" => "tYES"
  "ImportFileNum" => null
  "SummeryType" => "dNoSummary"
  "ContactPersonCode" => null
  "ShowSCN" => "tNO"
  "Series" => 182
  "TaxDate" => "2024-03-04T00:00:00Z"
  "PartialSupply" => "tYES"
  "DocObjectCode" => "oCreditNotes"
  "ShipToCode" => ""
  "Indicator" => null
  "FederalTaxID" => "03.104.892.9-725.000"
  "DiscountPercent" => 0.0
  "PaymentReference" => null
  "CreationDate" => "2024-03-04T00:00:00Z"
  "UpdateDate" => "2024-03-04T00:00:00Z"
  "FinancialPeriod" => 96
  "UserSign" => 31
  "TransNum" => 8054934
  "VatSum" => 112079.09
  "VatSumSys" => 112079.09
  "VatSumFc" => 0.0
  "NetProcedure" => "tNO"
  "DocTotalFc" => 0.0
  "DocTotalSys" => 1130979.89
  "Form1099" => null
  "Box1099" => null
  "RevisionPo" => "tNO"
  "RequriedDate" => null
  "CancelDate" => null
  "BlockDunning" => "tNO"
  "Submitted" => "tNO"
  "Segment" => 0
  "PickStatus" => "tNO"
  "Pick" => "tNO"
  "PaymentMethod" => null
  "PaymentBlock" => "tNO"
  "PaymentBlockEntry" => null
  "CentralBankIndicator" => null
  "MaximumCashDiscount" => "tNO"
  "Reserve" => "tNO"
  "Project" => null
  "ExemptionValidityDateFrom" => null
  "ExemptionValidityDateTo" => null
  "WareHouseUpdateType" => "dwh_Stock"
  "Rounding" => "tNO"
  "ExternalCorrectedDocNum" => null
  "InternalCorrectedDocNum" => null
  "NextCorrectingDocument" => null
  "DeferredTax" => "tNO"
  "TaxExemptionLetterNum" => null
  "WTApplied" => 0.0
  "WTAppliedFC" => 0.0
  "BillOfExchangeReserved" => "tNO"
  "AgentCode" => null
  "WTAppliedSC" => 0.0
  "TotalEqualizationTax" => 0.0
  "TotalEqualizationTaxFC" => 0.0
  "TotalEqualizationTaxSC" => 0.0
  "NumberOfInstallments" => 1
  "ApplyTaxOnFirstInstallment" => "tNO"
  "TaxOnInstallments" => "toiProportionally"
  "WTNonSubjectAmount" => 0.0
  "WTNonSubjectAmountSC" => 0.0
  "WTNonSubjectAmountFC" => 0.0
  "WTExemptedAmount" => 0.0
  "WTExemptedAmountSC" => 0.0
  "WTExemptedAmountFC" => 0.0
  "BaseAmount" => 0.0
  "BaseAmountSC" => 0.0
  "BaseAmountFC" => 0.0
  "WTAmount" => 0.0
  "WTAmountSC" => 0.0
  "WTAmountFC" => 0.0
  "VatDate" => null
  "DocumentsOwner" => null
  "FolioPrefixString" => null
  "FolioNumber" => null
  "DocumentSubType" => "bod_None"
  "BPChannelCode" => null
  "BPChannelContact" => null
  "Address2" => ""
  "DocumentStatus" => "bost_Open"
  "PeriodIndicator" => "Default"
  "PayToCode" => "Bill to"
  "ManualNumber" => null
  "UseShpdGoodsAct" => "tNO"
  "IsPayToBank" => "tNO"
  "PayToBankCountry" => null
  "PayToBankCode" => null
  "PayToBankAccountNo" => null
  "PayToBankBranch" => null
  "BPL_IDAssignedToInvoice" => 4
  "DownPayment" => 0.0
  "ReserveInvoice" => "tNO"
  "LanguageCode" => 8
  "TrackingNumber" => null
  "PickRemark" => ""
  "ClosingDate" => null
  "SequenceCode" => null
  "SequenceSerial" => null
  "SeriesString" => null
  "SubSeriesString" => null
  "SequenceModel" => "0"
  "UseCorrectionVATGroup" => "tNO"
  "TotalDiscount" => 0.0
  "DownPaymentAmount" => 0.0
  "DownPaymentPercentage" => 0.0
  "DownPaymentType" => "dptInvoice"
  "DownPaymentAmountSC" => 0.0
  "DownPaymentAmountFC" => 0.0
  "VatPercent" => 0.0
  "ServiceGrossProfitPercent" => 0.0
  "OpeningRemarks" => null
  "ClosingRemarks" => null
  "RoundingDiffAmount" => 0.0
  "RoundingDiffAmountFC" => 0.0
  "RoundingDiffAmountSC" => 0.0
  "Cancelled" => "tNO"
  "SignatureInputMessage" => null
  "SignatureDigest" => null
  "CertificationNumber" => null
  "PrivateKeyVersion" => null
  "ControlAccount" => "110301"
  "InsuranceOperation347" => "tNO"
  "ArchiveNonremovableSalesQuotation" => "tNO"
  "GTSChecker" => null
  "GTSPayee" => null
  "ExtraMonth" => 0
  "ExtraDays" => 21
  "CashDiscountDateOffset" => 0
  "StartFrom" => "pdt_None"
  "NTSApproved" => "tNO"
  "ETaxWebSite" => null
  "ETaxNumber" => null
  "NTSApprovedNumber" => null
  "EDocGenerationType" => "edocNotRelevant"
  "EDocSeries" => null
  "EDocNum" => null
  "EDocExportFormat" => null
  "EDocStatus" => "edoc_Ok"
  "EDocErrorCode" => null
  "EDocErrorMessage" => null
  "DownPaymentStatus" => "so_Open"
  "GroupSeries" => null
  "GroupNumber" => null
  "GroupHandWritten" => "tNO"
  "ReopenOriginalDocument" => "tNO"
  "ReopenManuallyClosedOrCanceledDocument" => null
  "CreateOnlineQuotation" => "tNO"
  "POSEquipmentNumber" => null
  "POSManufacturerSerialNumber" => null
  "POSCashierNumber" => null
  "ApplyCurrentVATRatesForDownPaymentsToDraw" => "tNO"
  "ClosingOption" => "coByCurrentSystemDate"
  "SpecifiedClosingDate" => null
  "OpenForLandedCosts" => "tYES"
  "AuthorizationStatus" => "dasWithout"
  "TotalDiscountFC" => 0.0
  "TotalDiscountSC" => 0.0
  "RelevantToGTS" => "tNO"
  "BPLName" => "SAMARINDA"
  "VATRegNum" => "MB003"
  "AnnualInvoiceDeclarationReference" => null
  "Supplier" => null
  "Releaser" => null
  "Receiver" => null
  "BlanketAgreementNumber" => null
  "IsAlteration" => "tNO"
  "CancelStatus" => "csNo"
  "AssetValueDate" => null
  "DocumentDelivery" => "ddtNoneSeleted"
  "AuthorizationCode" => null
  "StartDeliveryDate" => null
  "StartDeliveryTime" => null
  "EndDeliveryDate" => null
  "EndDeliveryTime" => null
  "VehiclePlate" => null
  "ATDocumentType" => null
  "ElecCommStatus" => null
  "ElecCommMessage" => null
  "ReuseDocumentNum" => "tNO"
  "ReuseNotaFiscalNum" => "tNO"
  "PrintSEPADirect" => "tNO"
  "FiscalDocNum" => null
  "POSDailySummaryNo" => null
  "POSReceiptNo" => null
  "PointOfIssueCode" => null
  "Letter" => null
  "FolioNumberFrom" => null
  "FolioNumberTo" => null
  "InterimType" => "boidt_None"
  "RelatedType" => -1
  "RelatedEntry" => null
  "SAPPassport" => null
  "DocumentTaxID" => null
  "DateOfReportingControlStatementVAT" => null
  "ReportingSectionControlStatementVAT" => null
  "ExcludeFromTaxReportControlStatementVAT" => "tNO"
  "POS_CashRegister" => null
  "UpdateTime" => "16:43:58"
  "CreateQRCodeFrom" => null
  "PriceMode" => null
  "OriginalRefNo" => null
  "OriginalRefDate" => null
  "Revision" => "tNO"
  "GSTTransactionType" => null
  "OriginalCreditOrDebitNo" => null
  "OriginalCreditOrDebitDate" => null
  "ShipFrom" => ""
  "CommissionTrade" => "ct_Empty"
  "CommissionTradeReturn" => "tNO"
  "UseBillToAddrToDetermineTax" => "tNO"
  "IssuingReason" => 1
  "Cig" => null
  "Cup" => null
  "EDocType" => "edocFE"
  "FCEAsPaymentMeans" => "tNO"
  "PaidToDate" => 0.0
  "PaidToDateFC" => 0.0
  "PaidToDateSys" => 0.0
  "FatherCard" => null
  "FatherType" => "cPayments_sum"
  "ShipState" => null
  "ShipPlace" => null
  "CustOffice" => null
  "FCI" => null
  "AddLegIn" => null
  "LegTextF" => null
  "DANFELgTxt" => null
  "IndFinal" => "tNO"
  "DataVersion" => 1
  "LastPageFolioNumber" => null
  "InventoryStatus" => "bost_Open"
  "PlasticPackagingTaxRelevant" => "tNO"
  "NotRelevantForMonthlyInvoice" => "tNO"
  "AllocationNumberIL" => null
  "U_REALISASIDODATE" => null
  "U_ALASANRETUR" => "0"
  "U_ALASANSOCANCEL" => "0"
  "U_PLATMOBIL" => null
  "U_DRIVER" => null
  "U_ALASANGR" => "0"
  "U_ALASANGI" => "0"
  "U_NOBILYETGIRO" => null
  "U_NOFAKTURPAJAK" => null
  "U_TGLFAKTURPAJAK" => null
  "U_ALMTFAKTURPAJAK" => null
  "U_IVNO" => null
  "U_IVDATE" => null
  "U_INDLVDATE" => null
  "U_INDUEDATE" => null
  "U_STATUS_EXPORTED" => null
  "U_VATODT" => null
  "U_VATOLOC" => null
  "U_VATADD" => null
  "U_VATONO" => null
  "U_SURATJALAN" => null
  "U_TIPEKIRIM" => "N"
  "U_TGLEXPIRED" => null
  "U_STATUS_PRINT" => "N"
  "U_BLITZ" => "N"
  "U_NOPOLISI" => "KT 8596 BM"
  "U_NOPOLISI2" => "KT 8809 MK"
  "U_RELEASE" => "N"
  "Document_ApprovalRequests" => []
  "DocumentLines" => array:3 [
    0 => array:258 [
      "LineNum" => 0
      "ItemCode" => "2601/00/POC/0001"
      "ItemDescription" => "POCKY STRAWBERRY HALF SIZE STICK BISCUIT 21 GR X 120"
      "Quantity" => 280.0
      "ShipDate" => null
      "Price" => 3000.9
      "PriceAfterVAT" => 3331.0
      "Currency" => "IDR"
      "Rate" => 0.0
      "DiscountPercent" => 0.0
      "VendorNum" => null
      "SerialNum" => null
      "WarehouseCode" => "SMDWHBS"
      "SalesPersonCode" => -1
      "CommisionPercent" => 0.0
      "TreeType" => "iNotATree"
      "AccountCode" => "411105"
      "UseBaseUnits" => "tNO"
      "SupplierCatNum" => null
      "CostingCode" => "SMD"
      "ProjectCode" => null
      "BarCode" => "8990044000079"
      "VatGroup" => "PPNO11"
      "Height1" => 0.0
      "Hight1Unit" => null
      "Height2" => 0.0
      "Height2Unit" => null
      "Lengh1" => 0.0
      "Lengh1Unit" => null
      "Lengh2" => 0.0
      "Lengh2Unit" => null
      "Weight1" => 0.0
      "Weight1Unit" => null
      "Weight2" => 0.0
      "Weight2Unit" => null
      "Factor1" => 1.0
      "Factor2" => 1.0
      "Factor3" => 1.0
      "Factor4" => 1.0
      "BaseType" => 16
      "BaseEntry" => 314420
      "BaseLine" => 0
      "Volume" => 0.0
      "VolumeUnit" => 4
      "Width1" => 0.0
      "Width1Unit" => null
      "Width2" => 0.0
      "Width2Unit" => null
      "Address" => ""
      "TaxCode" => "PPNO11"
      "TaxType" => "tt_Yes"
      "TaxLiable" => "tYES"
      "PickStatus" => "tNO"
      "PickQuantity" => 0.0
      "PickListIdNumber" => null
      "OriginalItem" => null
      "BackOrder" => null
      "FreeText" => null
      "ShippingMethod" => -1
      "POTargetNum" => null
      "POTargetEntry" => null
      "POTargetRowNum" => null
      "CorrectionInvoiceItem" => "ciis_ShouldBe"
      "CorrInvAmountToStock" => 0.0
      "CorrInvAmountToDiffAcct" => 0.0
      "AppliedTax" => 0.0
      "AppliedTaxFC" => 0.0
      "AppliedTaxSC" => 0.0
      "WTLiable" => "tYES"
      "DeferredTax" => "tNO"
      "EqualizationTaxPercent" => 0.0
      "TotalEqualizationTax" => 0.0
      "TotalEqualizationTaxFC" => 0.0
      "TotalEqualizationTaxSC" => 0.0
      "NetTaxAmount" => 92427.72
      "NetTaxAmountFC" => 0.0
      "NetTaxAmountSC" => 92427.72
      "MeasureUnit" => "BOX"
      "UnitsOfMeasurment" => 1.0
      "LineTotal" => 840252.0
      "TaxPercentagePerRow" => 11.0
      "TaxTotal" => 92427.72
      "ConsumerSalesForecast" => "tNO"
      "ExciseAmount" => 0.0
      "TaxPerUnit" => 0.0
      "TotalInclTax" => 0.0
      "CountryOrg" => null
      "SWW" => null
      "TransactionType" => null
      "DistributeExpense" => "tYES"
      "RowTotalFC" => 0.0
      "RowTotalSC" => 840252.0
      "LastBuyInmPrice" => 0.0
      "LastBuyDistributeSumFc" => 0.0
      "LastBuyDistributeSumSc" => 0.0
      "LastBuyDistributeSum" => 0.0
      "StockDistributesumForeign" => 0.0
      "StockDistributesumSystem" => 0.0
      "StockDistributesum" => 0.0
      "StockInmPrice" => 0.0
      "PickStatusEx" => "dlps_NotPicked"
      "TaxBeforeDPM" => 0.0
      "TaxBeforeDPMFC" => 0.0
      "TaxBeforeDPMSC" => 0.0
      "CFOPCode" => null
      "CSTCode" => null
      "Usage" => null
      "TaxOnly" => "tNO"
      "VisualOrder" => 0
      "BaseOpenQuantity" => 280.0
      "UnitPrice" => 3000.9
      "LineStatus" => "bost_Open"
      "PackageQuantity" => 280.0
      "Text" => null
      "LineType" => "dlt_Regular"
      "COGSCostingCode" => "SMD"
      "COGSAccountCode" => "511105"
      "ChangeAssemlyBoMWarehouse" => "N"
      "GrossBuyPrice" => 2643.19
      "GrossBase" => -5
      "GrossProfitTotalBasePrice" => 740092.45
      "CostingCode2" => "MIX"
      "CostingCode3" => "SAL"
      "CostingCode4" => null
      "CostingCode5" => null
      "ItemDetails" => null
      "LocationCode" => null
      "ActualDeliveryDate" => "2024-03-04T00:00:00Z"
      "RemainingOpenQuantity" => 280.0
      "OpenAmount" => 840252.0
      "OpenAmountFC" => 0.0
      "OpenAmountSC" => 840252.0
      "ExLineNo" => null
      "RequiredDate" => null
      "RequiredQuantity" => 0.0
      "COGSCostingCode2" => "MIX"
      "COGSCostingCode3" => "SAL"
      "COGSCostingCode4" => null
      "COGSCostingCode5" => null
      "CSTforIPI" => null
      "CSTforPIS" => null
      "CSTforCOFINS" => null
      "CreditOriginCode" => null
      "WithoutInventoryMovement" => "tNO"
      "AgreementNo" => null
      "AgreementRowNumber" => null
      "ActualBaseEntry" => null
      "ActualBaseLine" => null
      "DocEntry" => 447600
      "Surpluses" => 0.0
      "DefectAndBreakup" => 0.0
      "Shortages" => 0.0
      "ConsiderQuantity" => "tNO"
      "PartialRetirement" => "tNO"
      "RetirementQuantity" => 0.0
      "RetirementAPC" => 0.0
      "ThirdParty" => "tNO"
      "PoNum" => null
      "PoItmNum" => null
      "ExpenseType" => null
      "ReceiptNumber" => null
      "ExpenseOperationType" => null
      "FederalTaxID" => null
      "GrossProfit" => 100159.55
      "GrossProfitFC" => 0.0
      "GrossProfitSC" => 100159.55
      "PriceSource" => "dpsManual"
      "EnableReturnCost" => "tNO"
      "ReturnCost" => 0.0
      "ReturnAction" => 1
      "ReturnReason" => -1
      "StgSeqNum" => null
      "StgEntry" => null
      "StgDesc" => null
      "UoMEntry" => 61
      "UoMCode" => "BOX"
      "InventoryQuantity" => 280.0
      "RemainingOpenInventoryQuantity" => 280.0
      "ParentLineNum" => null
      "Incoterms" => 0
      "TransportMode" => 0
      "NatureOfTransaction" => null
      "DestinationCountryForImport" => null
      "DestinationRegionForImport" => null
      "OriginCountryForExport" => null
      "OriginRegionForExport" => null
      "ItemType" => "dit_Item"
      "ChangeInventoryQuantityIndependently" => "tNO"
      "FreeOfChargeBP" => "tNO"
      "SACEntry" => null
      "HSNEntry" => null
      "GrossPrice" => 3331.0
      "GrossTotal" => 932680.0
      "GrossTotalFC" => 0.0
      "GrossTotalSC" => 932680.0
      "NCMCode" => -1
      "NVECode" => null
      "IndEscala" => "tNO"
      "CtrSealQty" => 0.0
      "CNJPMan" => null
      "CESTCode" => null
      "UFFiscalBenefitCode" => null
      "ReverseCharge" => "tNO"
      "ShipToCode" => ""
      "ShipToDescription" => ""
      "OwnerCode" => null
      "ExternalCalcTaxRate" => 0.0
      "ExternalCalcTaxAmount" => 0.0
      "ExternalCalcTaxAmountFC" => 0.0
      "ExternalCalcTaxAmountSC" => 0.0
      "StandardItemIdentification" => 0
      "CommodityClassification" => 0
      "WeightOfRecycledPlastic" => 0.0
      "PlasticPackageExemptionReason" => null
      "LegalText" => null
      "UnencumberedReason" => null
      "CUSplit" => "tNO"
      "ListNum" => null
      "RecognizedTaxCode" => null
      "U_SIEBELID" => null
      "U_CNODN" => null
      "U_CNOVOUCHER" => null
      "U_CCUST" => null
      "U_CUSTNM" => null
      "U_NNILAI" => 0.0
      "U_ALASANUNSERVE" => null
      "U_DISC1" => 0.0
      "U_DISC2" => 0.0
      "U_DISC3" => 0.0
      "U_DISC4" => 0.0
      "U_DISC5" => 0.0
      "U_DISC6" => 0.0
      "U_DISC7" => 0.0
      "U_DISCVALUE1" => 0.0
      "U_DISCVALUE2" => 0.0
      "U_DISCVALUE3" => 0.0
      "U_DISCVALUE4" => 0.0
      "U_DISCVALUE5" => 0.0
      "U_DISCVALUE6" => 0.0
      "U_DISCVALUE7" => 0.0
      "U_DISCVALUE" => 0.0
      "U_BUNDLEQTY" => 0.0
      "U_BUNDLEPRICE" => 0.0
      "U_ITEMBONUS" => "N"
      "U_QTYKTN" => 0.0
      "U_INSTOCKWHS" => 0.0
      "U_NSIB" => 0.0
      "U_DISC8" => 0.0
      "U_DISCVALUE8" => 0.0
      "LineTaxJurisdictions" => []
      "ExportProcesses" => []
      "EBooksDetails" => []
      "DocumentLineAdditionalExpenses" => []
      "WithholdingTaxLines" => []
      "SerialNumbers" => []
      "BatchNumbers" => []
      "CCDNumbers" => []
      "DocumentLinesBinAllocations" => []
    ]
    1 => array:258 [
      "LineNum" => 1
      "ItemCode" => "2600/00/POC/0021"
      "ItemDescription" => "POCKY ALMOND FLAVOUR BISCUIT STICK 36GR X 120"
      "Quantity" => 25.0
      "ShipDate" => null
      "Price" => 5954.96
      "PriceAfterVAT" => 6610.01
      "Currency" => "IDR"
      "Rate" => 0.0
      "DiscountPercent" => 0.0
      "VendorNum" => null
      "SerialNum" => null
      "WarehouseCode" => "SMDWHBS"
      "SalesPersonCode" => -1
      "CommisionPercent" => 0.0
      "TreeType" => "iNotATree"
      "AccountCode" => "411105"
      "UseBaseUnits" => "tNO"
      "SupplierCatNum" => null
      "CostingCode" => "SMD"
      "ProjectCode" => null
      "BarCode" => "8851019000343"
      "VatGroup" => "PPNO11"
      "Height1" => 0.0
      "Hight1Unit" => null
      "Height2" => 0.0
      "Height2Unit" => null
      "Lengh1" => 0.0
      "Lengh1Unit" => null
      "Lengh2" => 0.0
      "Lengh2Unit" => null
      "Weight1" => 0.0
      "Weight1Unit" => null
      "Weight2" => 0.0
      "Weight2Unit" => null
      "Factor1" => 1.0
      "Factor2" => 1.0
      "Factor3" => 1.0
      "Factor4" => 1.0
      "BaseType" => 16
      "BaseEntry" => 314420
      "BaseLine" => 1
      "Volume" => 0.0
      "VolumeUnit" => 4
      "Width1" => 0.0
      "Width1Unit" => null
      "Width2" => 0.0
      "Width2Unit" => null
      "Address" => ""
      "TaxCode" => "PPNO11"
      "TaxType" => "tt_Yes"
      "TaxLiable" => "tYES"
      "PickStatus" => "tNO"
      "PickQuantity" => 0.0
      "PickListIdNumber" => null
      "OriginalItem" => null
      "BackOrder" => null
      "FreeText" => null
      "ShippingMethod" => -1
      "POTargetNum" => null
      "POTargetEntry" => null
      "POTargetRowNum" => null
      "CorrectionInvoiceItem" => "ciis_ShouldBe"
      "CorrInvAmountToStock" => 0.0
      "CorrInvAmountToDiffAcct" => 0.0
      "AppliedTax" => 0.0
      "AppliedTaxFC" => 0.0
      "AppliedTaxSC" => 0.0
      "WTLiable" => "tYES"
      "DeferredTax" => "tNO"
      "EqualizationTaxPercent" => 0.0
      "TotalEqualizationTax" => 0.0
      "TotalEqualizationTaxFC" => 0.0
      "TotalEqualizationTaxSC" => 0.0
      "NetTaxAmount" => 16376.14
      "NetTaxAmountFC" => 0.0
      "NetTaxAmountSC" => 16376.14
      "MeasureUnit" => "PCS"
      "UnitsOfMeasurment" => 1.0
      "LineTotal" => 148874.0
      "TaxPercentagePerRow" => 11.0
      "TaxTotal" => 16376.14
      "ConsumerSalesForecast" => "tNO"
      "ExciseAmount" => 0.0
      "TaxPerUnit" => 0.0
      "TotalInclTax" => 0.0
      "CountryOrg" => null
      "SWW" => null
      "TransactionType" => null
      "DistributeExpense" => "tYES"
      "RowTotalFC" => 0.0
      "RowTotalSC" => 148874.0
      "LastBuyInmPrice" => 0.0
      "LastBuyDistributeSumFc" => 0.0
      "LastBuyDistributeSumSc" => 0.0
      "LastBuyDistributeSum" => 0.0
      "StockDistributesumForeign" => 0.0
      "StockDistributesumSystem" => 0.0
      "StockDistributesum" => 0.0
      "StockInmPrice" => 0.0
      "PickStatusEx" => "dlps_NotPicked"
      "TaxBeforeDPM" => 0.0
      "TaxBeforeDPMFC" => 0.0
      "TaxBeforeDPMSC" => 0.0
      "CFOPCode" => null
      "CSTCode" => null
      "Usage" => null
      "TaxOnly" => "tNO"
      "VisualOrder" => 1
      "BaseOpenQuantity" => 25.0
      "UnitPrice" => 5954.96
      "LineStatus" => "bost_Open"
      "PackageQuantity" => 25.0
      "Text" => null
      "LineType" => "dlt_Regular"
      "COGSCostingCode" => "SMD"
      "COGSAccountCode" => "511105"
      "ChangeAssemlyBoMWarehouse" => "N"
      "GrossBuyPrice" => 5244.86
      "GrossBase" => -5
      "GrossProfitTotalBasePrice" => 131121.43
      "CostingCode2" => "MIX"
      "CostingCode3" => "SAL"
      "CostingCode4" => null
      "CostingCode5" => null
      "ItemDetails" => null
      "LocationCode" => null
      "ActualDeliveryDate" => "2024-03-04T00:00:00Z"
      "RemainingOpenQuantity" => 25.0
      "OpenAmount" => 148874.0
      "OpenAmountFC" => 0.0
      "OpenAmountSC" => 148874.0
      "ExLineNo" => null
      "RequiredDate" => null
      "RequiredQuantity" => 0.0
      "COGSCostingCode2" => "MIX"
      "COGSCostingCode3" => "SAL"
      "COGSCostingCode4" => null
      "COGSCostingCode5" => null
      "CSTforIPI" => null
      "CSTforPIS" => null
      "CSTforCOFINS" => null
      "CreditOriginCode" => null
      "WithoutInventoryMovement" => "tNO"
      "AgreementNo" => null
      "AgreementRowNumber" => null
      "ActualBaseEntry" => null
      "ActualBaseLine" => null
      "DocEntry" => 447600
      "Surpluses" => 0.0
      "DefectAndBreakup" => 0.0
      "Shortages" => 0.0
      "ConsiderQuantity" => "tNO"
      "PartialRetirement" => "tNO"
      "RetirementQuantity" => 0.0
      "RetirementAPC" => 0.0
      "ThirdParty" => "tNO"
      "PoNum" => null
      "PoItmNum" => null
      "ExpenseType" => null
      "ReceiptNumber" => null
      "ExpenseOperationType" => null
      "FederalTaxID" => null
      "GrossProfit" => 17752.57
      "GrossProfitFC" => 0.0
      "GrossProfitSC" => 17752.57
      "PriceSource" => "dpsManual"
      "EnableReturnCost" => "tNO"
      "ReturnCost" => 0.0
      "ReturnAction" => 1
      "ReturnReason" => -1
      "StgSeqNum" => null
      "StgEntry" => null
      "StgDesc" => null
      "UoMEntry" => 68
      "UoMCode" => "PCS"
      "InventoryQuantity" => 25.0
      "RemainingOpenInventoryQuantity" => 25.0
      "ParentLineNum" => null
      "Incoterms" => 0
      "TransportMode" => 0
      "NatureOfTransaction" => null
      "DestinationCountryForImport" => null
      "DestinationRegionForImport" => null
      "OriginCountryForExport" => null
      "OriginRegionForExport" => null
      "ItemType" => "dit_Item"
      "ChangeInventoryQuantityIndependently" => "tNO"
      "FreeOfChargeBP" => "tNO"
      "SACEntry" => null
      "HSNEntry" => null
      "GrossPrice" => 6610.01
      "GrossTotal" => 165250.25
      "GrossTotalFC" => 0.0
      "GrossTotalSC" => 165250.25
      "NCMCode" => -1
      "NVECode" => null
      "IndEscala" => "tNO"
      "CtrSealQty" => 0.0
      "CNJPMan" => null
      "CESTCode" => null
      "UFFiscalBenefitCode" => null
      "ReverseCharge" => "tNO"
      "ShipToCode" => ""
      "ShipToDescription" => ""
      "OwnerCode" => null
      "ExternalCalcTaxRate" => 0.0
      "ExternalCalcTaxAmount" => 0.0
      "ExternalCalcTaxAmountFC" => 0.0
      "ExternalCalcTaxAmountSC" => 0.0
      "StandardItemIdentification" => 0
      "CommodityClassification" => 0
      "WeightOfRecycledPlastic" => 0.0
      "PlasticPackageExemptionReason" => null
      "LegalText" => null
      "UnencumberedReason" => null
      "CUSplit" => "tNO"
      "ListNum" => null
      "RecognizedTaxCode" => null
      "U_SIEBELID" => null
      "U_CNODN" => null
      "U_CNOVOUCHER" => null
      "U_CCUST" => null
      "U_CUSTNM" => null
      "U_NNILAI" => 0.0
      "U_ALASANUNSERVE" => null
      "U_DISC1" => 0.0
      "U_DISC2" => 0.0
      "U_DISC3" => 0.0
      "U_DISC4" => 0.0
      "U_DISC5" => 0.0
      "U_DISC6" => 0.0
      "U_DISC7" => 0.0
      "U_DISCVALUE1" => 0.0
      "U_DISCVALUE2" => 0.0
      "U_DISCVALUE3" => 0.0
      "U_DISCVALUE4" => 0.0
      "U_DISCVALUE5" => 0.0
      "U_DISCVALUE6" => 0.0
      "U_DISCVALUE7" => 0.0
      "U_DISCVALUE" => 0.0
      "U_BUNDLEQTY" => 0.0
      "U_BUNDLEPRICE" => 0.0
      "U_ITEMBONUS" => "N"
      "U_QTYKTN" => 0.0
      "U_INSTOCKWHS" => 0.0
      "U_NSIB" => 0.0
      "U_DISC8" => 0.0
      "U_DISCVALUE8" => 0.0
      "LineTaxJurisdictions" => []
      "ExportProcesses" => []
      "EBooksDetails" => []
      "DocumentLineAdditionalExpenses" => []
      "WithholdingTaxLines" => []
      "SerialNumbers" => []
      "BatchNumbers" => []
      "CCDNumbers" => []
      "DocumentLinesBinAllocations" => []
    ]
    2 => array:258 [
      "LineNum" => 2
      "ItemCode" => "2600/00/POC/0005"
      "ItemDescription" => "POCKY COOKIES & CREAM STICK BISCUIT 40GR X 120"
      "Quantity" => 5.0
      "ShipDate" => null
      "Price" => 5954.96
      "PriceAfterVAT" => 6610.01
      "Currency" => "IDR"
      "Rate" => 0.0
      "DiscountPercent" => 0.0
      "VendorNum" => null
      "SerialNum" => null
      "WarehouseCode" => "SMDWHBS"
      "SalesPersonCode" => -1
      "CommisionPercent" => 0.0
      "TreeType" => "iNotATree"
      "AccountCode" => "411105"
      "UseBaseUnits" => "tNO"
      "SupplierCatNum" => null
      "CostingCode" => "SMD"
      "ProjectCode" => null
      "BarCode" => "8990044000024"
      "VatGroup" => "PPNO11"
      "Height1" => 0.0
      "Hight1Unit" => null
      "Height2" => 0.0
      "Height2Unit" => null
      "Lengh1" => 0.0
      "Lengh1Unit" => null
      "Lengh2" => 0.0
      "Lengh2Unit" => null
      "Weight1" => 0.0
      "Weight1Unit" => null
      "Weight2" => 0.0
      "Weight2Unit" => null
      "Factor1" => 1.0
      "Factor2" => 1.0
      "Factor3" => 1.0
      "Factor4" => 1.0
      "BaseType" => 16
      "BaseEntry" => 314420
      "BaseLine" => 2
      "Volume" => 0.0
      "VolumeUnit" => 4
      "Width1" => 0.0
      "Width1Unit" => null
      "Width2" => 0.0
      "Width2Unit" => null
      "Address" => ""
      "TaxCode" => "PPNO11"
      "TaxType" => "tt_Yes"
      "TaxLiable" => "tYES"
      "PickStatus" => "tNO"
      "PickQuantity" => 0.0
      "PickListIdNumber" => null
      "OriginalItem" => null
      "BackOrder" => null
      "FreeText" => null
      "ShippingMethod" => -1
      "POTargetNum" => null
      "POTargetEntry" => null
      "POTargetRowNum" => null
      "CorrectionInvoiceItem" => "ciis_ShouldBe"
      "CorrInvAmountToStock" => 0.0
      "CorrInvAmountToDiffAcct" => 0.0
      "AppliedTax" => 0.0
      "AppliedTaxFC" => 0.0
      "AppliedTaxSC" => 0.0
      "WTLiable" => "tYES"
      "DeferredTax" => "tNO"
      "EqualizationTaxPercent" => 0.0
      "TotalEqualizationTax" => 0.0
      "TotalEqualizationTaxFC" => 0.0
      "TotalEqualizationTaxSC" => 0.0
      "NetTaxAmount" => 3275.23
      "NetTaxAmountFC" => 0.0
      "NetTaxAmountSC" => 3275.23
      "MeasureUnit" => "BOX"
      "UnitsOfMeasurment" => 1.0
      "LineTotal" => 29774.8
      "TaxPercentagePerRow" => 11.0
      "TaxTotal" => 3275.23
      "ConsumerSalesForecast" => "tNO"
      "ExciseAmount" => 0.0
      "TaxPerUnit" => 0.0
      "TotalInclTax" => 0.0
      "CountryOrg" => null
      "SWW" => null
      "TransactionType" => null
      "DistributeExpense" => "tYES"
      "RowTotalFC" => 0.0
      "RowTotalSC" => 29774.8
      "LastBuyInmPrice" => 0.0
      "LastBuyDistributeSumFc" => 0.0
      "LastBuyDistributeSumSc" => 0.0
      "LastBuyDistributeSum" => 0.0
      "StockDistributesumForeign" => 0.0
      "StockDistributesumSystem" => 0.0
      "StockDistributesum" => 0.0
      "StockInmPrice" => 0.0
      "PickStatusEx" => "dlps_NotPicked"
      "TaxBeforeDPM" => 0.0
      "TaxBeforeDPMFC" => 0.0
      "TaxBeforeDPMSC" => 0.0
      "CFOPCode" => null
      "CSTCode" => null
      "Usage" => null
      "TaxOnly" => "tNO"
      "VisualOrder" => 2
      "BaseOpenQuantity" => 5.0
      "UnitPrice" => 5954.96
      "LineStatus" => "bost_Open"
      "PackageQuantity" => 5.0
      "Text" => null
      "LineType" => "dlt_Regular"
      "COGSCostingCode" => "SMD"
      "COGSAccountCode" => "511105"
      "ChangeAssemlyBoMWarehouse" => "N"
      "GrossBuyPrice" => 5246.61
      "GrossBase" => -5
      "GrossProfitTotalBasePrice" => 26233.07
      "CostingCode2" => "MIX"
      "CostingCode3" => "SAL"
      "CostingCode4" => null
      "CostingCode5" => null
      "ItemDetails" => null
      "LocationCode" => null
      "ActualDeliveryDate" => "2024-03-04T00:00:00Z"
      "RemainingOpenQuantity" => 5.0
      "OpenAmount" => 29774.8
      "OpenAmountFC" => 0.0
      "OpenAmountSC" => 29774.8
      "ExLineNo" => null
      "RequiredDate" => null
      "RequiredQuantity" => 0.0
      "COGSCostingCode2" => "MIX"
      "COGSCostingCode3" => "SAL"
      "COGSCostingCode4" => null
      "COGSCostingCode5" => null
      "CSTforIPI" => null
      "CSTforPIS" => null
      "CSTforCOFINS" => null
      "CreditOriginCode" => null
      "WithoutInventoryMovement" => "tNO"
      "AgreementNo" => null
      "AgreementRowNumber" => null
      "ActualBaseEntry" => null
      "ActualBaseLine" => null
      "DocEntry" => 447600
      "Surpluses" => 0.0
      "DefectAndBreakup" => 0.0
      "Shortages" => 0.0
      "ConsiderQuantity" => "tNO"
      "PartialRetirement" => "tNO"
      "RetirementQuantity" => 0.0
      "RetirementAPC" => 0.0
      "ThirdParty" => "tNO"
      "PoNum" => null
      "PoItmNum" => null
      "ExpenseType" => null
      "ReceiptNumber" => null
      "ExpenseOperationType" => null
      "FederalTaxID" => null
      "GrossProfit" => 3541.73
      "GrossProfitFC" => 0.0
      "GrossProfitSC" => 3541.73
      "PriceSource" => "dpsManual"
      "EnableReturnCost" => "tNO"
      "ReturnCost" => 0.0
      "ReturnAction" => 1
      "ReturnReason" => -1
      "StgSeqNum" => null
      "StgEntry" => null
      "StgDesc" => null
      "UoMEntry" => 61
      "UoMCode" => "BOX"
      "InventoryQuantity" => 5.0
      "RemainingOpenInventoryQuantity" => 5.0
      "ParentLineNum" => null
      "Incoterms" => 0
      "TransportMode" => 0
      "NatureOfTransaction" => null
      "DestinationCountryForImport" => null
      "DestinationRegionForImport" => null
      "OriginCountryForExport" => null
      "OriginRegionForExport" => null
      "ItemType" => "dit_Item"
      "ChangeInventoryQuantityIndependently" => "tNO"
      "FreeOfChargeBP" => "tNO"
      "SACEntry" => null
      "HSNEntry" => null
      "GrossPrice" => 6610.01
      "GrossTotal" => 33050.05
      "GrossTotalFC" => 0.0
      "GrossTotalSC" => 33050.05
      "NCMCode" => -1
      "NVECode" => null
      "IndEscala" => "tNO"
      "CtrSealQty" => 0.0
      "CNJPMan" => null
      "CESTCode" => null
      "UFFiscalBenefitCode" => null
      "ReverseCharge" => "tNO"
      "ShipToCode" => ""
      "ShipToDescription" => ""
      "OwnerCode" => null
      "ExternalCalcTaxRate" => 0.0
      "ExternalCalcTaxAmount" => 0.0
      "ExternalCalcTaxAmountFC" => 0.0
      "ExternalCalcTaxAmountSC" => 0.0
      "StandardItemIdentification" => 0
      "CommodityClassification" => 0
      "WeightOfRecycledPlastic" => 0.0
      "PlasticPackageExemptionReason" => null
      "LegalText" => null
      "UnencumberedReason" => null
      "CUSplit" => "tNO"
      "ListNum" => null
      "RecognizedTaxCode" => null
      "U_SIEBELID" => null
      "U_CNODN" => null
      "U_CNOVOUCHER" => null
      "U_CCUST" => null
      "U_CUSTNM" => null
      "U_NNILAI" => 0.0
      "U_ALASANUNSERVE" => null
      "U_DISC1" => 0.0
      "U_DISC2" => 0.0
      "U_DISC3" => 0.0
      "U_DISC4" => 0.0
      "U_DISC5" => 0.0
      "U_DISC6" => 0.0
      "U_DISC7" => 0.0
      "U_DISCVALUE1" => 0.0
      "U_DISCVALUE2" => 0.0
      "U_DISCVALUE3" => 0.0
      "U_DISCVALUE4" => 0.0
      "U_DISCVALUE5" => 0.0
      "U_DISCVALUE6" => 0.0
      "U_DISCVALUE7" => 0.0
      "U_DISCVALUE" => 0.0
      "U_BUNDLEQTY" => 0.0
      "U_BUNDLEPRICE" => 0.0
      "U_ITEMBONUS" => "N"
      "U_QTYKTN" => 0.0
      "U_INSTOCKWHS" => 0.0
      "U_NSIB" => 0.0
      "U_DISC8" => 0.0
      "U_DISCVALUE8" => 0.0
      "LineTaxJurisdictions" => []
      "ExportProcesses" => []
      "EBooksDetails" => []
      "DocumentLineAdditionalExpenses" => []
      "WithholdingTaxLines" => []
      "SerialNumbers" => []
      "BatchNumbers" => []
      "CCDNumbers" => []
      "DocumentLinesBinAllocations" => []
    ]
  ]
  "EWayBillDetails" => []
  "ElectronicProtocols" => []
  "DocumentAdditionalExpenses" => []
  "WithholdingTaxDataWTXCollection" => []
  "WithholdingTaxDataCollection" => []
  "DocumentSpecialLines" => []
  "DownPaymentsToDraw" => []
  "TaxExtension" => array:57 [
    "TaxId0" => null
    "TaxId1" => null
    "TaxId2" => null
    "TaxId3" => null
    "TaxId4" => null
    "TaxId5" => null
    "TaxId6" => null
    "TaxId7" => null
    "TaxId8" => null
    "TaxId9" => null
    "State" => null
    "County" => null
    "Incoterms" => null
    "Vehicle" => null
    "VehicleState" => null
    "NFRef" => "Return from ERP - C.E.00000124"
    "Carrier" => null
    "PackQuantity" => null
    "PackDescription" => null
    "Brand" => null
    "ShipUnitNo" => null
    "NetWeight" => 0.0
    "GrossWeight" => 0.0
    "StreetS" => null
    "BlockS" => null
    "BuildingS" => null
    "CityS" => null
    "ZipCodeS" => null
    "CountyS" => null
    "StateS" => null
    "CountryS" => null
    "StreetB" => "JL. S. PARMAN MAL LEMBUSWANA BLOK A NO.11 - 12"
    "BlockB" => "PALARAN"
    "BuildingB" => ""
    "CityB" => "SAMARINDA"
    "ZipCodeB" => null
    "CountyB" => "RAWA MAKMUR"
    "StateB" => "001"
    "CountryB" => "ID"
    "ImportOrExport" => null
    "MainUsage" => null
    "GlobalLocationNumberS" => null
    "GlobalLocationNumberB" => null
    "TaxId12" => null
    "TaxId13" => null
    "BillOfEntryNo" => null
    "BillOfEntryDate" => null
    "OriginalBillOfEntryNo" => null
    "OriginalBillOfEntryDate" => null
    "ImportOrExportType" => "et_IpmortsOrExports"
    "PortCode" => null
    "DocEntry" => 447600
    "BoEValue" => 0.0
    "ClaimRefund" => null
    "DifferentialOfTaxRate" => null
    "IsIGSTAccount" => null
    "TaxId14" => null
  ]
  "AddressExtension" => array:59 [
    "ShipToStreet" => null
    "ShipToStreetNo" => null
    "ShipToBlock" => null
    "ShipToBuilding" => null
    "ShipToCity" => null
    "ShipToZipCode" => null
    "ShipToCounty" => null
    "ShipToState" => null
    "ShipToCountry" => null
    "ShipToAddressType" => null
    "BillToStreet" => "JL. S. PARMAN MAL LEMBUSWANA BLOK A NO.11 - 12"
    "BillToStreetNo" => null
    "BillToBlock" => "PALARAN"
    "BillToBuilding" => ""
    "BillToCity" => "SAMARINDA"
    "BillToZipCode" => null
    "BillToCounty" => "RAWA MAKMUR"
    "BillToState" => "001"
    "BillToCountry" => "ID"
    "BillToAddressType" => null
    "ShipToGlobalLocationNumber" => null
    "BillToGlobalLocationNumber" => null
    "ShipToAddress2" => null
    "ShipToAddress3" => null
    "BillToAddress2" => null
    "BillToAddress3" => null
    "PlaceOfSupply" => null
    "PurchasePlaceOfSupply" => null
    "DocEntry" => 447600
    "GoodsIssuePlaceBP" => null
    "GoodsIssuePlaceCNPJ" => null
    "GoodsIssuePlaceCPF" => null
    "GoodsIssuePlaceStreet" => null
    "GoodsIssuePlaceStreetNo" => null
    "GoodsIssuePlaceBuilding" => null
    "GoodsIssuePlaceZip" => null
    "GoodsIssuePlaceBlock" => null
    "GoodsIssuePlaceCity" => null
    "GoodsIssuePlaceCounty" => null
    "GoodsIssuePlaceState" => null
    "GoodsIssuePlaceCountry" => null
    "GoodsIssuePlacePhone" => null
    "GoodsIssuePlaceEMail" => null
    "GoodsIssuePlaceDepartureDate" => null
    "DeliveryPlaceBP" => null
    "DeliveryPlaceCNPJ" => null
    "DeliveryPlaceCPF" => null
    "DeliveryPlaceStreet" => null
    "DeliveryPlaceStreetNo" => null
    "DeliveryPlaceBuilding" => null
    "DeliveryPlaceZip" => null
    "DeliveryPlaceBlock" => null
    "DeliveryPlaceCity" => null
    "DeliveryPlaceCounty" => null
    "DeliveryPlaceState" => null
    "DeliveryPlaceCountry" => null
    "DeliveryPlacePhone" => null
    "DeliveryPlaceEMail" => null
    "DeliveryPlaceDepartureDate" => null
  ]
  "DocumentReferences" => []
]