            ...
            <form method="POST" id="sync">
              <div class="row mb-2">
                ...
                <div class="col-sm-3 d-flex">
                  ...
                  <a href="javascript:void(0);" class="btn btn-info print mr-2 ml-2">
                    Cetak
                  </a>
                  <button class="btn btn-success mr-2" type="button" onclick="exportExcel()">Excel</button>
                  <!-- <button class="btn btn-success mr-2" type="button" onclick="unservedPng()">Excel SAP</button> -->
                </div>
              </div>
            </form>
          </div>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $("#sync").on("submit",function(e){
      ...
      var dateT = $("#dateT").val();
      var csrf = "{{ csrf_token() }}";
      var url = '{{ route('report.unserved_order.png_search') }}';
      $.ajax({
        url : url,
  ...
  function exportExcel() {
    ...
    var url = `/report/unserved_order/png_export?branch_code=${branch_code}&dateF=${dateF}&dateT=${dateT}`;
    window.location = url
  }

  function unservedPng() {
    $('#overlay').show();
      var branch_code = $("#branch_code").val();
      var dateF = $("#dateF").val();
      var dateT = $("#dateT").val();
      var csrf = "{{ csrf_token() }}";
      var url = '{{ route('report.unserved_order.unserved_png_sap') }}';
      $.ajax({
        url : url,
        data  : {branch_code:branch_code,dateF:dateF,dateT:dateT,_token:csrf},
        type : "POST",
