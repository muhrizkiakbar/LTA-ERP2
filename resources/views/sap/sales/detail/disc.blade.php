<div class="modal-dialog modal-xl">
  <div class="modal-content">
    ...
    <form method="POST" action="{{ route('sales.discount_update') }}">
      ...
      <table class="table table-xxs table-striped table-bordered">
        ...
        <tbody>
          ...
          <input type="hidden" name="total[]" value="{{ $item['beforeDiscount'] }}">
					...
					<input type="hidden" name="qty_eko[]" value="{{ $item['qty_eko'] }}">
					<input type="hidden" name="qty_total[]" value="{{ $item['qty_total'] }}">
...
{{-- <script type="text/javascript">
  $(document).ready(function(){
    $("#discx").on("submit",function(e){
      ...
      $("#modalEx").modal('hide');
      $('#overlay').fadeIn();
      var url = '{{ route('sales.discount_update') }}';
      $.ajax({
        url: url,
        type: "POST",
